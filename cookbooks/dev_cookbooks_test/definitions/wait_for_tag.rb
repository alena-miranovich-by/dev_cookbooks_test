...
define :wait_for_tag, :collection_name => nil, :should_exist => true, :timeout => TIMEOUT_SEC do
  ...
  ruby_block "wait for tag #{state}." do
    block do
      ...
      if (Chef::VERSION =~ /^(0\.)?(9|10|11)/) # provider signature changed in Chef 0.9 or later
        ...
        status = Timeout::timeout(timeout) do
          until done
            ...
            unless done
              ...
              printable = {}
              h.keys.each { |k| printable[k.to_s] = h[k].inspect }
              Chef::Log.info "  node[:server_collection][#{collection}] = #{printable.inspect}"
              sleep 10
            end
          end
        end
